  ...
  return (
    <div className="relative w-full h-screen min-h-[660px] bg-[#FAF7F2] flex flex-col justify-between overflow-hidden select-none">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#1C1917]/90 via-[#1C1917]/40 to-black/20" />
      </div>
      ...
      >
        <div className="bg-black/35 backdrop-blur-md border-none inline-flex items-center gap-3 px-4 py-2.5 rounded-2xl shadow-lg">
          ...
          <div>
            <h1 className="font-display font-extrabold text-xs text-white tracking-[0.2em] uppercase">
              DUMP CAFE
            </h1>
      ...
      {/* Bottom Welcome Card — Borderless */}
      <div className="relative z-10 p-5 pb-8">
        <motion.div 
          initial={{ opacity: 0, y: 35 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.3, type: "spring", stiffness: 220, damping: 24 }}
          className="bg-[#1C1917]/95 backdrop-blur-xl border-none p-6 rounded-[32px] text-white shadow-2xl space-y-4"
        >
          {/* Badge */}
          <div className="inline-flex items-center gap-1.5 bg-[#C85A32]/25 text-[#FF9E79] border-none text-[10px] font-brand font-bold px-3.5 py-1 rounded-full uppercase tracking-wider">
            <Sparkles className="w-3 h-3 text-amber-300" />
            <span>SELAMAT DATANG DI DUMP CAFE</span>
          </div>

          {/* Title with Modern Syne Display Font */}
          <div className="space-y-1">
            <div className="flex flex-wrap gap-x-2">
              {words.map((word, i) => (
                <span key={i} className="font-display font-extrabold text-2xl text-white leading-tight">
                  {word}
                </span>
              ))}
            </div>
              ...
              {words2.map((word, i) => (
                <span
                  ...
                  className={`font-display font-extrabold text-2xl leading-tight ${
                    word === 'Your' || word === 'Table' ? 'text-[#FF9E79] italic font-normal' : 'text-white'
                  }`}
                >
          ...
          <button
            ...
            className="btn-cta w-full bg-gradient-to-r from-[#C85A32] to-[#E8703E] hover:from-[#A44321] hover:to-[#C85A32] text-white font-brand font-extrabold text-xs py-4 px-6 rounded-2xl flex items-center justify-center gap-2 shadow-xl shadow-[#C85A32]/40 tracking-wider uppercase border-none"
          >
